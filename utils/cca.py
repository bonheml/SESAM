...
def cross_covariance(x, y):
    ...
    :param x: the first matrix of size T x N
    :type x: np.array
    :param y: the second matrix of size T x M
    :type y: np.array
...
def compute_cca(x, y):
    ...
    :param x: the matrix X of size T x N
    :type x: np.array
    :param y: the matrix Y of size T x M
    :type y: np.array
    ...
    c_xx, c_xy, c_yy = cross_covariance(x, y)

    #  Compute c_xx^-1/2 and c_yy^-1/2
    c_xx_sqrt_inv = matrix_sqrt_inv(c_xx)
    c_yy_sqrt_inv = matrix_sqrt_inv(c_yy)
    ...
    # Reduce the dimensionality of U and S to the number of correlations coefficients found
